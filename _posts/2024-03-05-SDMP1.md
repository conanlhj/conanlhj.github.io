...
$$
G^*_5 \rarr G^*_4 \rarr G^*_3 \rarr G^*_2 \rarr G^*_1
$$
...
이제, e로 도착하는 action을 찾아보면 $d \rarr e$ 가 있다. 이외의 가능한 action으 존재하지 않으므로, 해당 action만 신경쓰면 된다. 따라서, $x_4=d$ 이고, $u_4=d \rarr e$ 일 때, $G^*_4(d)=l(d, d \rarr e) + G^*_5(e)=1+0=1$ 이 된다. 이를 통해, 위 표를 업데이트하면 다음과 같다.
...
> 왜 $G^*_4(e) = \infty$ 일까?
> 
> e에서 e로 도달하는 방법이 없기 때문이다. 우리는 지금 Fixed-Length Planning을 하고 있으므로, 현재의 State에서 별다를 action을 취하지 않고 가만히 있는다는 선택지는 존재하지 않는다. 가만히 있는다는 termination action이 존재하는 unFixed-Length Planning에서는 $G^*_4(e)$가 0이 될 수 있다.
...
$$
- $x_3=c$, $u_3=c \rarr d$
   - $G^*_3(c)=l(c, c \rarr d) + G^*_4(d)=1+1=2$
- $x_3=b$, $u_3=b \rarr d$
   - $G^*_3(b)=l(b, b \rarr d) + G^*_4(d)=4+1=5$
