...
이제, e로 도착하는 action을 찾아보면 $d \rightarrow e$ 가 있다. 이외의 가능한 action으 존재하지 않으므로, 해당 action만 신경쓰면 된다. 따라서, $x_4=d$ 이고, $u_4=d \rightarrow e$ 일 때, $G^*_4(d)=l(d, d \rightarrow e) + G^*_5(e)=1+0=1$ 이 된다. 이를 통해, 위 표를 업데이트하면 다음과 같다.
